...
# OUTPUT: 
# `results/Cohort2_FindMarkers_DARs_tfbs_annotated.rds`
...
# load data ----
input <- readRDS(here("data", "Cohort2_FindMarkers_DARs.rds"))
...
annotated.lola.tib_subset <- annotated.lola.tib_all %>%
    filter(tf_class %in% c("ZNF143", "CTCF", "SMC1A", "SMC3", "STAG1", "RAD21")) %>%
    mutate(tf_class = factor(tf_class, levels = c("ZNF143", "CTCF", "SMC1A", "SMC3", "STAG1", "RAD21")))

lola.stats <- t_test_log2fc(annotated.lola.tib_all,
                            features = levels(annotated.lola.tib_all$tf_class),
                            feature.col = "tf_class")

saveRDS(annotated.lola.tib_all, here("results", "Cohort2_FindMarkers_DARs_tfbs_annotated.rds"))
